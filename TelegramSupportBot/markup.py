...
def markup_main(user_id):
    ...
    if user_role != "agent":
        item1 = types.KeyboardButton("✏️ Надіслати запит")
        markup_main.row(item1)

    if user_role == "agent":
        item2 = types.KeyboardButton("Повернутись до головного меню")
        markup_main.row(item2)
    else:
...
def markup_agent():
    ...
    item1 = types.InlineKeyboardButton("❗️ Очікують відповіді від підтримки", callback_data='waiting_reqs:1')
    item2 = types.InlineKeyboardButton("⏳ Очікують відповіді від користувача", callback_data='answered_reqs:1')
    item3 = types.InlineKeyboardButton("✅ Закриті запити", callback_data='confirm_reqs:1')
...
def markup_admin():
    ...
    item1 = types.InlineKeyboardButton("✅ Додати агента підтримки", callback_data='add_agent')
    item2 = types.InlineKeyboardButton("🧑‍💻 Агенти підтримки", callback_data='all_agents:1')
    item3 = types.InlineKeyboardButton("🔑 Одноразові паролі", callback_data='all_passwords:1')
    item4 = types.InlineKeyboardButton("🎲 Сгенерирувати одноразові паролі", callback_data='generate_passwords')
    item5 = types.InlineKeyboardButton("⛔️ Вимкнути бота", callback_data='stop_bot:wait')
...
def markup_request_action(req_id, req_status, callback):
    ...
    if req_status == 'confirm':
        item1 = types.InlineKeyboardButton("🗂 Показати файли", callback_data=f'req_files:{req_id}:{callback}:1')
        ...
        item1 = types.InlineKeyboardButton("✏️ Додати повідомлення", callback_data=f'add_message:{req_id}:{status_user}')
        item2 = types.InlineKeyboardButton("🗂 Показати файли", callback_data=f'req_files:{req_id}:{callback}:1')

        if status_user == 'user':
            item3 = types.InlineKeyboardButton("✅ Закрити запит", callback_data=f'confirm_req:wait:{req_id}')
            item4 = types.InlineKeyboardButton("Назад", callback_data=formatted_callback)
            markup_request_action.add(item1, item2, item3, item4)
        elif status_user == 'agent':
            item3 = types.InlineKeyboardButton("✅ Закрити запит", callback_data=f'confirm_req:wait:{req_id}')
            item4 = types.InlineKeyboardButton("Назад", callback_data=formatted_callback)
            markup_request_action.add(item1, item2, item3, item4)
...
def markup_confirm_req(req_id):
    ...
    item1 = types.InlineKeyboardButton("✅ Підтвердити", callback_data=f'confirm_req:true:{req_id}')
...
def markup_confirm_stop():
    ...
    item1 = types.InlineKeyboardButton("Так", callback_data='stop_bot:confirm')
    item2 = types.InlineKeyboardButton("Ні", callback_data='back_admin')
